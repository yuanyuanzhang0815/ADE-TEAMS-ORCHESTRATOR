...
默认最大循环次数不超过 3 次。除非用户明确要求，不生成超过 5 次的循环。
...
适用：摘要、简单文案、单轮分析、单文档生成。

### 14.2 文档生产类

```text
start -> ADE-资料分析 -> ADE-文档撰写 -> ADE-质量评审 -> end
```
...
```text
start -> ADE-需求理解 -> ADE-技术方案 -> ADE-代码实现 -> ADE-测试验证 -> end
```

适用：功能开发、Bug 修复、接口改造、工程优化。
